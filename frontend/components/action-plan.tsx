...
export function ActionPlan() {
  ...
  return <section aria-labelledby="action-plan-heading" className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
    ...
    {loading || loadedFor !== user ? <p className="mt-6 text-sm text-slate-500">Görevler yükleniyor...</p> : tasks.length === 0 ? <p className="mt-6 text-sm text-slate-500">Henüz görev yok.</p> : <ul className="mt-6 space-y-3">{tasks.map((task) => <li key={task.id} className="rounded-xl border border-slate-200 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><input aria-label="Görev başlığı" value={task.title} disabled={busy} onChange={(event) => setTasks((current) => current.map((item) => item.id === task.id ? { ...item, title: event.target.value } : item))} onBlur={(event) => { if (event.target.value.trim()) void changeTask(task, { title: event.target.value }); }} className="w-full border-0 p-0 font-medium text-slate-950 outline-none" /><textarea aria-label="Görev açıklaması" value={task.description || ""} disabled={busy} placeholder="Açıklama eklenmedi." onChange={(event) => setTasks((current) => current.map((item) => item.id === task.id ? { ...item, description: event.target.value } : item))} onBlur={(event) => void changeTask(task, { description: event.target.value || null })} className="mt-1 min-h-12 w-full resize-y border-0 p-0 text-sm text-slate-600 outline-none" /></div><select aria-label="Görev durumu" value={task.status} disabled={busy} onChange={(event) => void changeTask(task, { status: event.target.value as ActionPlanStatus })} className="rounded-lg border border-slate-300 px-2 py-1 text-sm"><option value="todo">Yapılacak</option><option value="in_progress">Devam ediyor</option><option value="done">Tamamlandı</option></select><button type="button" disabled={busy} onClick={() => void removeTask(task.id)} className="text-sm font-medium text-red-700 underline">Sil</button></div></li>)}</ul>}
  </section>;
}
